...
export function buildPrompt(input: ResponderInput): string {
  return [
    ...
    '   anything outside it.',
    '3. Answer helpfully and concisely from those files. Be THOROUGH: list the',
    '   project files and check the relevant ones before answering. NEVER claim',
    '   something is missing/absent from a partial look — verify across the files',
    '   first. If they genuinely do not cover it, say so rather than guessing.',
